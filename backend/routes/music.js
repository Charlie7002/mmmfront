...
router.get('/getMusic/:id', async function (req, res, next) {
	...
	for (let i = 0; i < 3; i++) {
		tracksTop.push({
			...
			cover: tracks.data.artist.discography.topTracks.items[i].track.album.coverArt.sources[1]
				.url,
	...
	for (let i = 0; i < tracks.data.artist.discography.albums.items.length; i++) {
		albums.push({
			title: tracks.data.artist.discography.albums.items[i].releases.items[0].name,
			cover: tracks.data.artist.discography.albums.items[i].releases.items[0].coverArt.sources[0]
				.url,
			artist: music.tracks[0].artists[0].name,
		});
	}
